...
<head>
    ...
    <script type="text/javascript">
        ...
        function itemclick(item) {
            ...
            if ('add' === item.id) {
                window.top.openDialog("添加功能","${path}/maintain/yeji/addUI?fenlei_id=${fenlei_id}",80,80, [
                ...
                window.top.openDialog("修改功能","${path}/maintain/yeji/addUI?fenlei_id=${fenlei_id}&id="+rows.id,80,80, [
                    { text: '确定', onclick: function (item, dialog) {window.top.submitForm()},cls:'l-dialog-btn-highlight' },
                    { text: '取消', onclick: function (item, dialog) { window.top.closeDialog(); }}
                ]);
                }else if ('del' === item.id) {
                ...
                $.ligerDialog.confirm("确定删除此数据么？",function(yes){
                    if(yes){
                        $.getJSON("${path}/maintain/yeji/del", {id: rows.id}, function (data) {
