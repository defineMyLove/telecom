...
var _ie6 = window.ActiveXObject && !window.XMLHttpRequest,
	...
	isMobile = !!navigator.userAgent.match(/mobile/i),//rengeng：是否是移动设备
...
//#rengeng#修改		if( me.substr(me.lastIndexOf('/')).indexOf('lhgdialog') !== -1 )
//		    break;
	    if( me.substr(me.lastIndexOf('/')).indexOf('sea.js') !== -1 )
		    break;
	}
	
	me = me.split('?'); _args = me[1];
	
	//#rengeng#修改 return me[0].substr( 0, me[0].lastIndexOf('/') + 1 );
	return me[0].substr( 0, me[0].lastIndexOf('/') + 1 )+'jquery-plugin/lhgdialog/';
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	content: function( msg, add, frm )
	{
		...
		}else if(msg instanceof $){//start#rengeng#内容支持jquery dom,关闭时放回到原来的位置
				if(that.$contentConfig){
					that.$contentConfig.content.hide().appendTo(that.$contentConfig.parent);//先回复dom到原来位置
				}
				that.$contentConfig={parent:msg.parent(),content:msg};
				$content.append(msg.show());
		}//end#rengeng#jquery dom
		else{
			$content.html( msg );
		}
		// 新增内容后调整位置
	...
	position: function( left, top )
	{
		var that = this,
			...
			dt = isFixed ? 0 : docTop,
			ww = isMobile?_$doc.width():_$top.width(),//rengeng：相对于文档而并非可视区域_$top变为_$doc
			wh = isMobile?_$doc.height():_$top.height(),
			ow = wrap.offsetWidth,
	...
	focus: function()
	{
	    try{
		    ...
		    elemFocus = this._focus && this._focus[0]|| this.DOM.title[0];
	...
	close: function()
	{
		...
		//#rengeng#start获取上一级dialog ,用于关闭后让上一级窗口获取焦点
		var openerApi=that.config.parent;
		//#rengeng#end获取上一级dialog
		
		if(that.$contentConfig){//start#rengeng#内容支持jquery dom,关闭时放回到原来的位置
			that.$contentConfig.content.hide().appendTo(that.$contentConfig.parent);//先回复dom到原来位置
		}//end#rengeng#jquery dom
		...
		if( that.iframe )
		{
			...
			if( that._frmTimer ) window.clearTimeout(that._frmTimer);
		}
		else
		{
		...
		that.closed = true;
		//#rengeng#上一级窗口获取焦点start
		openerApi&&openerApi.zindex().focus();
		//#rengeng#获取焦点end
		return that;
	},
	...
	max: function()
	{
		...
		if( !that._maxState )
		{
			...
			if( that.iframe){//rengeng：处理iframe宽度sess _iframe()
				var paddingLeft=Number(that.DOM.content.css('padding-left').replace('px',""));
				var paddingRight=Number(that.DOM.content.css('padding-right').replace('px',""));
				$(that.iframe).width(maxSize.w-paddingLeft-paddingRight);
			}
		}
		else
		{
			if( that.iframe ){//rengeng：处理iframe宽度sess _iframe()
				var paddingLeft=Number(that.DOM.content.css('padding-left').replace('px',""));
				var paddingRight=Number(that.DOM.content.css('padding-right').replace('px',""));
				$(that.iframe).width(that._or.w.replace('px',"")-paddingLeft-paddingRight);
			}
	...
	_iframe: function( url )
	{
		...
		that._frmTimer = window.setTimeout(function(){
		    $iframe.attr('src', url);
		}, 1);
		...
		var load = that._fmLoad = function()
		{
			...
			try{
				...
				//$ibody.css('overflow','hidden');//#rengeng# 
			}catch(e){// 跨域
			...
			var __width=iWidth;//rengeng:替换100%为实际宽度，可能会允许超出屏幕..（为了适应在移动设备等小屏幕上完整展示）
			
			if( typeof iWidth === 'number' ){//rengeng获取padding
				iWidth+=paddingLeft+paddingRight;
			}else{
				iWidth=Number(iWidth.replace('px',""))+paddingLeft+paddingRight;
			}
			if( typeof iHeight === 'number' ){//rengeng获取padding
				iHeight+=paddingTop+paddingBottom;
			}else{
				iHeight=Number(iHeight.replace('px',""))+paddingTop+paddingBottom;
			}
			
			// rengeng:适应iframe尺寸
			window.setTimeout(function(){
				if( typeof __width !== 'number' ){
					__width=__width.replace('px',"");
				}
				iframe.style.cssText = 'width:'+__width+'px;height:100%;border:none 0;';
				if( !that._maxState )
				{
			...
			/*rengeng:替换为实际宽度，见上window.setTimeout(function(){
			    iframe.style.cssText = loadCss;
			},0);*/// window.setTimeout: 防止IE6~7对话框样式渲染异常
		
			// 窗口最大化时这里不用再计算窗口的尺寸和位置了，如果再计算窗口会出现错位
			/*rengeng 删除，见上if( !that._maxState )
			{
			    that.size( iWidth, iHeight )
			    .position( config.left, config.top );
			}*/
	...
	_maxSize: function()
	{
	    var that = this, DOM = that.DOM,
			...
			main = DOM.main[0],
			maxWidth, maxHeight;
		var base = !!isMobile?_$doc:_$top;//rengeng：相对于文档的高度和宽度_$top-》_$doc
		maxWidth = base.width() - wrap.offsetWidth + main.offsetWidth;
		maxHeight = base.height() - wrap.offsetHeight + main.offsetHeight;
	...
	_addEvent: function()
	{
		...
		that._winResize = function()
		{
			...
			resizeTimer = window.setTimeout(that._reset, 140);
		};
		...
		DOM.wrap.bind('click', function(event){
			...
			{
				//that._click('cancel');
				//rengeng：关闭按钮触发config自定义myClose()事件(如果有的话)
				if(!!config.myClose){
					if(config.myClose.call(that, window) !== false){
						that.close();
					}
				}else{
					that._click('cancel');
...
/**
 * #rengeng#多种格式的警告、type:'alert','success','error'
 */
lhgdialog.alertMore=function(type,msg,onclose,parent){
	var icon='alert.gif';
	if(type=='alert'){
		icon='alert.gif';
 ...
 */
lhgdialog.confirm = function( content, yes, no,myClose, parent )
{
	return lhgdialog({
		title: '确认',
		id: 'confirm.gif',
		...
		myClose:function(here){//rengeng：加入自定义的右上角关闭按钮回调
			return myClose && myClose.call(this, here);
		}
	});
};
	...
	return lhgdialog({
		...
		content: [
			...
			'<div>',
				'<input value="',
					value,
				'" style="width:500px;padding:6px 4px" />',
			'</div>'
			].join(''),
		init: function(){
...
})( jQuery,win.lhgdialog);
(function(config){
    config['lock'] = true;
    config['resize'] = false;
    config['drag'] = false;
    config['cache'] = false;//for debug
    config['min'] = false;
    config['max'] = false;
    config['padding'] = '10px';
    // [more..]
})(jQuery.dialog.setting);
});