seajs.config({
    alias: {
        '$': 'jquery/jquery/1.8.0/jquery',
        'nivo-slider':'jquery-plugin/nivo-slider/jquery.nivo.slider.pack',
        'artDialog':'artDialog/jquery.artDialog.min.js',
        'ztree':'jquery-plugin/ztree/3.5.15/js/jquery.ztree.all-3.5.min',
        'jquery-util':'module/jqueryUtil.js',
        'pupTab':'module/pupTab/pupTab.js',
        'tip':'arale/tip/1.2.0/tip',
        'ligerui':'ligerui/1.2.0/js/ligerui.all.js',
        'popup':'arale/popup/1.1.5/popup',
        'pupZtree':'module/pupZtree',
        'calendar':'arale/calendar/0.9.0/calendar',
        'dialog':'arale/dialog/1.2.4/dialog',
        'upload':'arale/upload/1.0.1/upload',
        'tabs':'arale/switchable/1.0.1/tabs',
        'position':'arale/position/1.0.1/position'
    }
});