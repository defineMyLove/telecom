...
var cityList = {
    ...
    var CitySelect = function (param) {
        ...
        $(this.woEle).insertAfter('#' + this.triggerId);
        $('#'+this.tabDivEleId).append($(this.divEle)).append($(this.contentBoxEle));
        //直辖市
        var tabDivEleHtml = '<li ><a href="javascript:void(0)" name="">直辖市</a></li>';
        $('#'+this.uitabDivId).find('ul').append(tabDivEleHtml);
        var contentBoxEleHtmlId ='content_'+ this.contentBoxId+0;
        var contentBoxEleHtml = '<div style="display: none;" class="panel" id="'+contentBoxEleHtmlId+'"><div class="fenleilist clearfix"></div></div>';
        $('#'+this.contentBoxId).append(contentBoxEleHtml)
        $.each(cityList.municipalities, function (j) {
            var node = this;
            var contentBoxEleHtmlTid  = 'municipalitie' +j;
            var contentBoxEleHtmlT = '<span><a href="javascript:void(0)"  id="' +contentBoxEleHtmlTid + '">' + this.n + '</a></span>';
            $('#'+contentBoxEleHtmlId).find('.fenleilist').append(contentBoxEleHtmlT);
            var tt =  $('#'+contentBoxEleHtmlTid);
            tt.data('node',node);
        });
        //省份
        var tabDivEleHtml = '<li ><a href="javascript:void(0)" name="">全国各省</a></li>';
        $('#'+this.uitabDivId).find('ul').append(tabDivEleHtml);
        var contentBoxEleHtmlId ='content_'+ this.contentBoxId+1;
        var contentBoxEleHtml = '<div style="display: none;" class="panel"  id="'+contentBoxEleHtmlId+'"></div>';
        $('#'+this.contentBoxId).append(contentBoxEleHtml)
        $.each(cityList.provinces, function (i,province) {
        	var provinceHtml='<div class="province"><a href="javascript:void(0)" class="province_'+i+'">'+province.n+'</a></div><div class="fenleilist clearfix" style="display:none;" id="province'+i+'"></div>';
        	$('#'+contentBoxEleHtmlId).append(provinceHtml);
            $.each(province.cities, function (j) {
                var node = this;
                var contentBoxEleHtmlTid  = 'city_' + i+j;
                var contentBoxEleHtmlT = '<span><a href="javascript:void(0)"  id="' +contentBoxEleHtmlTid + '">' + this.n + '</a></span>';
                $('#province'+i).append(contentBoxEleHtmlT);
                var tt =  $('#'+contentBoxEleHtmlTid);
                tt.data('node',node);
            });
            //省份折叠
            $('.province_'+i).toggle(function(){$('#province'+i).show()},function(){$('#province'+i).hide()});
        });
        //香港其他
        var tabDivEleHtml = '<li ><a href="javascript:void(0)" name="">其他</a></li>';
        $('#'+this.uitabDivId).find('ul').append(tabDivEleHtml);
        var contentBoxEleHtmlId ='content_'+ this.contentBoxId+2;
        var contentBoxEleHtml = '<div style="display: none;" class="panel"  id="'+contentBoxEleHtmlId+'"><div class="fenleilist clearfix"></div></div>';
        $('#'+this.contentBoxId).append(contentBoxEleHtml)
        $.each(cityList.other, function (j) {
            var node = this;
            var contentBoxEleHtmlTid  = 'other_'+j;
            var contentBoxEleHtmlT = '<span><a href="javascript:void(0)"  id="' +contentBoxEleHtmlTid + '">' + this.n + '</a></span>';
            $('#'+contentBoxEleHtmlId).find('.fenleilist').append(contentBoxEleHtmlT);
            var tt =  $('#'+contentBoxEleHtmlTid);
            tt.data('node',node);
        ...
        var tem =
            new Tabs({
                ...
                panels: '.ui-switchable-content .panel',
                activeIndex: 0
            });
        var popup = new Popup({
		...
		$('#'+this.contentBoxId + ' a').click(function(){
		    ...
		    that.customClick(node.n,node.g);
		    popup.hide();
		});
    };
