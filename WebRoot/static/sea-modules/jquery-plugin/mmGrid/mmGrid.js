    ...
    MMGrid.prototype = {
            ...
            var mmGrid = [
                '<div class="mmGrid">',
                    ...
                    '<div class="mmg-loading">',
                        '<div class="mmg-loadingImg" style="float:left;"></div>',
                        '<div class="mmg-loadingText" style="float: left;line-height: 45px;height: 45px;">'+ this.opts.loadingText +'</div>',
                    '</div>',
        ...
        , _initEvents: function(){
            ...
            if(opts.width === 'auto' || opts.height === 'auto' || (typeof opts.width === 'string' && opts.width.indexOf('%') === opts.width.length-1) ||
            	...
            	$(window).on('resize', function(){
                    that.resize();
                    //#rengeng：加入重置宽度填充#
                    //无效that._setColsWidth();
                    if(that.opts.fullWidthRows){
                    	that._fullWidthRows();
                    }
                    //#rengeng#
                });
        ...
        , _fullWidthRows: function(){
            ...
            for(var i=0; i< leafCol.length; i++){
                var col = leafCol[i];
                var $th = $ths.eq(i);
                if(!col.lockWidth && $th.css('display')!='none'){//rengeng:visible存在问题，如果初始化表格时隐藏的，而此列是需要显示的
                    thsArr.push($th);
                }
            }
        ...
        , _showLoading: function(){
            ...
            //rengeng
            var left=($mmGrid.width() - $loading.width()) / 2,
        ...
        , _loadNative: function(args){
        	var data = args;
        	if(args.list){//add:#rengeng,20140429#加载本地数据，数据在args.list
        		data = args.list;
        	}
            this._populate(data);
        ...
        , load: function(args){
            try{
                ...
                }else if(args.list){//add:#rengeng,20140429#加载本地数据，数据在args.list
                	this._loadNative(args);
                }else if(opts.url){
                    this._loadAjax(args);
                }else if(opts.items){
