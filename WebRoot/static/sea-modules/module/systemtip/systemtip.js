...
system_tip.addFunctionTip=function(){
    ...
    $('#div_functionTip').css({display:'block'});
};
system_tip.addTip=function(obj){
    ...
    var ele =['<div id="div_', obj.id,'" class="tip"',
        'style="display:none;width:',obj.width,'px;height: ',obj.height,'px;right: ',
        rightPx,'px;">'];
    ele.push('<div class="panelbarbutton" id="panelbar_'+obj.id+'">');
    if(obj.img){
        ele.push('<img id="img_',obj.id,'" width="" height="" class="icon"/>');
    }
    ele.push('</div></div>');
...
system_tip.init=function(id,path){
    ...
    jQuery(document).ajaxStop(function(evt, request, settings) {
        //添加显示，隐藏系统提醒功能
       //TODO zxl--暂时不需要 system_tip.addFunctionTip();
    ...
    jQuery.getJSON(obj.url+"?curTTTTtimestamp="+new Date().getTime(),function(data){
            if(data.count!==0){
                ...
                if(countEle.length!==0){
                    ...
                    jQuery('#panelbar_'+obj.id).append([obj.title,'<span id="count_',obj.id,'">',data.count,'</span>'].join(''));
                }
                var level ="info";
                if(obj.level){
                    level= obj.level;
                ...
                if(winEle.length!==0){
                    ...
                    liList.push('<li id="li_',obj.id,'_',content.id,'" class="ellipsis"><a href=",content.url,">',content.title,'</a></li>');
                    ...
                    jQuery('#div_'+obj.id)
                        .append(
                            (function(){
                                var liList = ['<div class="window"><div><h4>',obj.langTitle,'</h4><menu></menu></div><ul>'];
                                jQuery.each(data.list,function(i,content){
                                    var con='<span class="label label-info">'+(i+1)+'</span>'+content.title;
                                    liList.push('<li id="li_',obj.id,'_',content.id,'" class="ellipsis"><a href="',obj.contentUrl,'">',content.title,'</a></li>');
                                });
                ...
                //显示tip
                jQuery('#div_'+obj.id).css({display:"block"});
            }else{
