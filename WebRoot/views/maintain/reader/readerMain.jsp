...
<head>
    ...
    <script type="text/javascript">
        ...
        seajs.use(['$', 'ligerui'], function ($, ligerui) {
            ...
            $("#usergrid").ligerGrid({
                ...
                columns: [
                    ...
                    { display: '位置', name: 'address', align: 'left'},
                    { display: '对应IP地址', name: 'ip', align: 'left'}
                ],
                url: '${path}/maintain/reader/list',
                root: 'rows',
                record: 'total',
                toolbar: { items: [
        ...
        function itemclick(item) {
            ...
            if ('add' === item.id) {
                window.top.openDialog("添加功能","${path}/maintain/reader/addUI",80,80, [
                ...
                window.top.openDialog("添加功能","${path}/maintain/reader/addUI?id="+rows.id,80,80, [
                    { text: '确定', onclick: function (item, dialog) {window.top.submitForm()},cls:'l-dialog-btn-highlight' },
                    { text: '取消', onclick: function (item, dialog) { window.top.closeDialog(); }}
                ]);
                }else if ('del' === item.id) {
                ...
                $.ligerDialog.confirm("确定删除此数据么？",function(yes){
                    if(yes){
                        $.getJSON("${path}/maintain/reader/del", {id: rows.id}, function (data) {
                ...
                window.top.openDialog("详情","${path}/maintain/reader/detail?id="+rows.id,80,80, [
                    { text: '取消', onclick: function (item, dialog) { window.top.closeDialog(); }}
                ]);
            }
        }
